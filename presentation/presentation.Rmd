...
Single variable: $RMSE = `r round(collect_metrics(games_play_fit_rs)$mean[1], 3)`$

Double variable: $RMSE = `r round(collect_metrics(games_play_all_fit_rs)$mean[1], 3)`$
...
# train_test_metrics %>% 
#   kable()
display_model_statistics <- function() {
	paste0(
		"$$RMSE=",
		train_test_metrics$test[1],
		",R^2=",
		train_test_metrics$test[2],
		".$$"
	)
}
```

`r display_model_statistics()`
