...
As seen in the boxplots, games that are more highly rated tend to have a longer playing time. We hope to investigate how different factors affect how highly a game is rated and use this to predict the rating of a board game. 
...
Here we can see that the plot has a modal value just less than 7.5, this may be due to the fact that this is the average rating for all games rather than the number of ratings affecting this value whatsoever. In our actual analysis we will need to find a way to avoid this issue and properly visualize the relationship between the two variables.
...
In order to reach our conclusions we will need to model each individual variable against the average rating and use statistical tests such as the chi-squared test in order to evaluate the fit of each model. We will also use correlation coefficient tests where the strength of the relationship is tested. We will use modeling to eliminate the influence of the distribution of the average ratings on our graph plotting the number of ratings against the average ratings.